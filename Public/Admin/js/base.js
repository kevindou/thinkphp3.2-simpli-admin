...
// ajax出现错误调用
function ajaxFail() {
    return layer.msg("服务器繁忙,请稍候再试...", {time: 1000, icon: 2})
}
...
function array_unique(arr) {
    var array = [];
    for (var i in arr) {
        if (!in_array(arr[i], array)) array.push(arr[i]);
    }
    return array
}

// 连接参数为字符串
function handleParams(params, prefix) {
    var other = "";
...
// 生成Input
function createInput(params, type) {
    return '<input type="' + type + '" ' + handleParams(params) + " />"
}

// 生成text
function createText(params) {
    return createInput(params, 'text')
}

// 生成密码
function createPassword(params) {
    return createInput(params, 'password')
}

// 生成textarea
function createTextarea(params) {
    if (empty(params)) {
        params = {"class": " form-control", "rows": 5}
    } else {
        params["class"] += " form-control";
        params["rows"] = 5
    }
    return "<textarea " + handleParams(params) + "></textarea>"
}
...
function createLang(obj) {
    ...
    if (language) {
        ...
        for (var i in language) {
            obj.name = n;
            obj.name += '[' + i + ']';
            var params = handleParams(obj), mid = n + m,
                input = obj.type == undefined ? "<textarea " + params + "></textarea>" : "<input type=\"text\" " + params + "/>";
...
function gAlert(sTitle, sMessage, className, iTime) {
    className = className ? className : "warning";
    iTime = iTime ? iTime : 1500; // 默认值
    return $.gritter.add({
        title: sTitle,
        text: sMessage,
        class_name: 'gritter-' + className + ' gritter-center',
        time: iTime
    });
}
...
function InitForm(select, data) {
    ...
    if (objForm != undefined) {
        ...
        $(objForm).find('input[type=checkbox]').each(function () {
            ...
            array_checkbox.push($(this).attr('name').replace("[]", ""));
        });                                                                             // 多选菜单
        $(objForm).find('input[type=radio]').each(function () {
            $(this).prop('checked', false).parent('span').removeClass('checked');
            radios.push($(this).attr('name'));
        });
        objForm.reset();                                                                // 表单重置
        radios = array_unique(radios);
        array_checkbox = array_unique(array_checkbox);
        if (data != undefined) {
            for (var i in data) {
                // 多语言处理 以及多选配置
                    ...
                    for (var x in data[i]) {
                        ...
                        if (objForm[key] != undefined)
                            ...
                            if (parseInt(data[i][x]) > 0) {
                                $('input[type=checkbox][value=' + data[i][x] + ']').attr('checked', true).each(function () {
                                    this.checked = true;
                ...
                if (objForm[i] != undefined && objForm[i].type != "password") {
                    ...
                    objForm[i].value = tmp;
                    if (in_array(i, radios)) {
                        $(objForm).find('input[type=radio][name=' + i + '][value=' + tmp + ']').prop("checked", true).parent('span').addClass('checked')
                    }


                } else {
                    if (in_array(i, array_checkbox)) {
                        $(objForm).find('input[type=checkbox][value=' + data[i] + ']').prop("checked", true).parent('span').addClass('checked')
                    }
                }
            }
        }
