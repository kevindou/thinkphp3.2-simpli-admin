...
<script type="text/javascript">
    $(function() {
        // 表格
        var table = showDataTable($('#showTable'), <?php echo ($aoColnmns); ?>, 10, '?m=<?php echo (MODULE_NAME); ?>&a=getDatas', <?php echo (($arrParams)?($arrParams):"undefined"); ?>);
        // 分页样式
        ...
        $('#updateUser').click(function(){
            showDiv($('#userDiv'), '修改导入用户lUserId', function(){
                return handleEditData($('#updateUserForm'), '?m=<?php echo (MODULE_NAME); ?>&a=updateUser', function(){
                    return false;
                }, true);
            })
        });
        ...
        $('.m-sort').click(function(){
            ...
            // 请求服务器获取数据
            $.ajax({
                url:'?m=<?php echo (MODULE_NAME); ?>&a=getSort',
                type:'get',
                dataType:'json',
                success:function(json)
                {
                    ...
                    if (json.status == 1)
                    {
                        ...
                        showDiv($('#sortDiv'), '推荐<?php echo ($title); ?>排序', function () {
                            ...
                            // 添加处理
                            $.ajax({
                                url:'?m=<?php echo (MODULE_NAME); ?>&a=setSort',
                                type:'post',
