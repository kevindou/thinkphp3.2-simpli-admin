...
class RoleController extends Controller
{
    ...
    public function beforeSearch(&$query)
    {
        ...
        if ($uid != 1)
        {
            ...
            $query = $query->andFilterWhere(['name' => $name]);
        }
    ...
    public function actionCreate($name)
    {
        ...
        // 判断自己是否有这个权限
        $uid   = Yii::$app->user->id;
        $roles = Yii::$app->getAuthManager()->getAssignment($name, $uid);
        if ( ! $roles && $uid != 1) throw new \yii\web\UnauthorizedHttpException('对不起，您没有修改该角色的权限!');
        ...
        $have  = $model->menus ? explode(',', $model->menus) : [];
        ...
        if ($menus)
        {
            ...
            foreach ($menus as $value)
            {
                ...
                $array = ['name' => $value->menu_name, 'id' => $value->id, 'type' => 'item'];

                // 默认选中
                if (in_array($value->id, $have)) $array['additionalParameters'] = ['item-selected' => true];
    ...
    public function actionView($name)
    {
        ...
        if ($model->menus)
        {
            $child = Menu::find()->where(['id' => explode(',', $model->menus)])->all();
            if ($child)
            {
                // 处理数据
